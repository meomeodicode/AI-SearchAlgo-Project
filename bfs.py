...
def breadth_first_search(grid, start, stones, switches):
    ...
    while frontier:
        ...
        ares_pos, stone_positions = current_state

        if is_goal_state(stone_positions, switches):
            end_time = time.time()
            memory_used = tracemalloc.get_traced_memory()[1] / (1024 ** 2)  
            tracemalloc.stop()
            return path, len(explored), end_time - start_time, memory_used
        ...
        for direction in directions:
            new_ares_pos = (ares_pos[0] + direction[0], ares_pos[1] + direction[1])
            
            if new_ares_pos in stone_positions:
                stone_index = stone_positions.index(new_ares_pos)
                new_stone_pos = (new_ares_pos[0] + direction[0], new_ares_pos[1] + direction[1])
                
                if is_valid_push(ares_pos, new_ares_pos, direction, grid, stone_positions):
                    new_stone_positions = list(stone_positions)
                    new_stone_positions[stone_index] = new_stone_pos  
                    new_state = (new_ares_pos, tuple(new_stone_positions))  
                    new_path = path + move_mapping[(direction[0], direction[1], "push")]
                    frontier.append((new_state, new_path))
                    
            elif is_valid_move(new_ares_pos, grid, stone_positions):
                new_state = (new_ares_pos, stone_positions)  
